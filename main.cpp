...
{
    Suit rankTest = Clubs;
    cout << "Ranktest is : " << static_cast<Suit>(rankTest) << endl;;
    ...
    for (int numGames = 0; numGames < 1000; numGames++)
    {
        ...
        while (playerA_Status && playerB_Status)
        {
            ...
            cout << "Player A placed a " << playerA_Card.getRank() << " of " << playerA_Card.getSuit() << endl;
            cout << "Player B placed a " << playerB_Card.getRank() << " of " << playerB_Card.getSuit() << endl;
            cout << endl;
            if (compareDifference(playerA_Card, playerB_Card))
            {
                if (playerA_Card.getRank() < playerB_Card.getRank()){
