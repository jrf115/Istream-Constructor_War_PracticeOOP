...
void fillRank(Deck& aDeck, int iter, int cardSuit)
{
    ...
    {
        aDeck[iter].setSuit (static_cast<Suit>(cardSuit));
        aDeck[iter].setRank (static_cast<Rank>(rankCounter));
        rankCounter++;
    }
}
...
{
    if (aCard.getRank() != bCard.getRank())
...
void war(Deck& playerA_Deck, Deck& playerB_Deck)
{
    ...
    {    // Each player needs to place down three cards vvv
        ...
        cout << "Player A's third card is a " << playerA_LastCard->getRank() << " of " << playerA_LastCard->getSuit() << endl;
        cout << "Player B's third card is a " << playerB_LastCard->getRank() << " of " << playerB_LastCard->getSuit() << endl;
        cout << endl;
        if (playerA_LastCard->getSuit() > playerB_LastCard->getSuit())
        {
            cout << "Player A wins B's cards " << endl;
            giveCardsToPlayer(playerA_Deck, playerA_PlayCards, playerB_PlayCards);
            return;
        }
        else if (playerB_LastCard->getSuit() > playerA_LastCard->getSuit())
        {
